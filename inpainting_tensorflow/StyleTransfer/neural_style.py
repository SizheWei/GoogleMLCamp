...
def stylize(content_img,style_n,model,style_i,output_image,savepath,cuda):
    device = torch.device("cuda" if cuda else "cpu")

    content_image = utils.load_image(content_img, scale=None)
    ...
    output = model(content_image, style_id = [style_i]).cpu()

    utils.save_image(savepath+'/'+output_image+'_style'+str(style_i)+'.png', output[0])
